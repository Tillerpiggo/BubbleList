...
print("The variable foo is defined at a local scope and can't be accessed outside of that scope.")
...
print("x is defined at a global scope, so it can be accessed from anywhere within the program (below it's definition). It is modified at a local scale, but there is no variable shadowing, so it maintains the same value in and out of that scope.")
/*:
...
greeting(greeting: "Hello", name: "Akash")
greeting(greeting: nil, name: "Akash")
/*:
